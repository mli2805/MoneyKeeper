...
namespace Keeper2018
{
    public class TranFilter
    {
        ...
        private bool FilterAmount()
        {
            ...
            if (_filterModel.AmountEqualTo) return double.Parse(_filterModel.Amount).Equals(_wrappedTran.Tran.Amount);
            if (_filterModel.AmountLessThan) return double.Parse(_filterModel.Amount) > _wrappedTran.Tran.Amount;
            // if (_filterModel.AmountGreaterThan) 
            return double.Parse(_filterModel.Amount) < _wrappedTran.Tran.Amount;
        }
