...
namespace KeeperDomain
{
    public static class TxtParser
    {
        ...
        public static PayCard CardFromString(this string s)
        {
            ...
            card.Serial = substrings[2].Trim();
            card.StartDate = DateTime.ParseExact(substrings[3].Trim(), "dd.MM.yyyy", CultureInfo.InvariantCulture);
            card.FinishDate = DateTime.ParseExact(substrings[4].Trim(), "dd.MM.yyyy", CultureInfo.InvariantCulture);

            card.CardNumber = substrings[5].Trim();
            card.CardHolder = substrings[6].Trim();
            card.IsMine = Convert.ToBoolean(substrings[7]);
            card.PaymentSystem = (PaymentSystem)Enum.Parse(typeof(PaymentSystem), substrings[8]);
            card.IsVirtual = Convert.ToBoolean(substrings[9]);
            card.IsPayPass = Convert.ToBoolean(substrings[10]);

            card.ShortName = substrings[11].Trim();
            card.Comment = substrings[12].Replace("|", "\r\n");
            return card;
        }
