...
namespace KeeperDomain.Basket
{
    public static class BelBaskets
    {
        ...
        {
            new BasketWeights() {Index = 1, DateFrom = new DateTime(2009, 1, 1), Usd = 1.0 / 3, Euro = 1.0 / 3, Rur = 1.0 / 3},
            new BasketWeights() {Index = 2, DateFrom = new DateTime(2016, 1, 1), Usd = 0.3, Euro = 0.3, Rur = 0.4}, // дата примерно
            new BasketWeights() {Index = 3, DateFrom = new DateTime(2016, 11, 1), Usd = 0.3, Euro = 0.2, Rur = 0.5},
            new BasketWeights() {Index = 4, DateFrom = new DateTime(2022, 7, 15), Usd = 0.3, Euro = 0.1, Rur = 0.5, Cny = 0.1},
            new BasketWeights() {Index = 5, DateFrom = new DateTime(2022, 12, 12), Usd = 0.3, Euro = 0, Rur = 0.6, Cny = 0.1},
        };
        ...
        public static double CalculateByLastRules(OfficialRates officialRates, out int number)
        {
            var weights = Weights.First(b => b.DateFrom == new DateTime(2022, 12, 12));

            number = Weights.LastOrDefault(w => w.DateFrom <= officialRates.Date)?.Index ?? 0;
